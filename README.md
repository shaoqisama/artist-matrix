...
- "Creation Workbench" adds a persona-aware chat loop and editable draft so you can shape Suno prompts (with revisions saved to `data/artists/<slug>/drafts/`). Configure DeepSeek keys to get AI suggestions; otherwise a local heuristic responds. Use "Finalize draft" to preview the Suno payload before rendering.
