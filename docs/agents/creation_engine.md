...
Run `artist-matrix chat --agent creation_engine --scope <artist-slug>`. The Codex-native agent reads
the artist and track manifests through MCP, co-creates a user-reviewed `TrackIdeationDraft`, and may
create only a pending `propose_track_render` action. Provider dispatch occurs only after host
approval. The service wiring below remains the deterministic execution/legacy adapter.
...
- `src/artist_matrix/creation_engine/llm.py`: Optional DeepSeek-powered ideation chat client and helpers (loads packaged `src/artist_matrix/prompts/creation/ideation_system.md`).
