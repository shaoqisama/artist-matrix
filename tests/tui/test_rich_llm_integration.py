...
def test_workbench_flow_integration(mock_llm):
    ...
    response_text, fields_dict, json_text = apply_llm_guidance(
        mock_llm, persona_summary, "create a simple track", [], draft
    )
...
def test_mock_llm_error_handling(mock_llm):
    ...
    profile = ArtistProfile(
        name="Test Artist", lyric_style="Electronic pop", visual_style="Neon colors"
    )
    ...
    response_text, fields_dict, json_text = apply_llm_guidance(
        mock_llm, persona_summary, "error test", [], draft
    )
...
def test_mock_llm_malformed_response(mock_llm):
    ...
    response_text, fields_dict, json_text = apply_llm_guidance(
        mock_llm, persona_summary, "return malformed response", [], draft
    )
