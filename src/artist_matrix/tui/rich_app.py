...
class RichTuiApp:
    ...
    def show_loading(self, message: str) -> None:
        ...
        with Progress(
            ...
            task = progress.add_task(f"{self.theme.weyland_yutani_tag} {message}...", total=None)
    ...
    def handle_generate(self) -> None:
        ...
        profile: ArtistProfile | None = (
            profile_obj if isinstance(profile_obj, ArtistProfile) else None
        )
        avatar: AvatarBlueprint | None = (
            avatar_obj if isinstance(avatar_obj, AvatarBlueprint) else None
        )
    ...
    def _collect_persona_draft(self) -> PersonaWizardDraft | None:
        ...
        instructions_panel = Panel(
            Text(
                "Enter '<' to go back, '.' to keep the current value, or provide new input.",
                style=self.theme.info_style,
            ),
        ...
        while True:
            ...
            while index < len(steps):
                ...
                styled_prompt.append(
                    f"Step {index + 1}/{len(steps)}: {prompt}", style=self.theme.info_style
                )
                ...
                if raw in {"<", "b", "back"}:
                    if index == 0:
                        self.console.print(
                            "  Already at the first step.", style=self.theme.accent_style
                        )
                        continue
                    index -= 1
                    continue
                ...
                if not raw:
                    if required and not current_value:
                        self.console.print(
                            "  Please provide a value.", style=self.theme.warning_style
                        )
                        continue
                    if kind == "sequence":
            ...
            while True:
                ...
                action_prompt.append(
                    f"{self.theme.weyland_yutani_tag} Action ", style=self.theme.warning_style
                )
                action_prompt.append(
                    "[f]orge / [r]efine / [e]dit / [c]ancel", style=self.theme.info_style
                )
                ...
                if action in {"e", "edit"}:
                    ...
                    if target.isdigit():
                        ...
                        if 0 <= idx < len(steps):
                            ...
                            self.console.print(
                                "  Invalid step number.", style=self.theme.warning_style
                            )
                            continue
                    else:
                        index = 0
    ...
    def _refine_with_ai(self, draft: PersonaWizardDraft) -> PersonaWizardDraft | None:
        ...
        if getattr(generator, "prompt_agent", None) is None:
            self.console.print(
                "  No AI provider configured; using heuristic fallback.",
                style=self.theme.info_style,
            )
        ...
        self.console.print(
            Panel(
                "Enter refinement instructions for the AI. Type 'done' when finished.",
                title="[WY] AI Refinement Mode",
                border_style=self.theme.border_style,
            )
        )
        ...
        while True:
            ...
            instruction_prompt.append(
                "Refinement note (blank to finish, 'c' to cancel)", style=self.theme.info_style
            )
            ...
            if not instruction or instruction.lower() == "done":
                if not instructions:
                    self.console.print(
                        "  No instructions provided; skipping refinement.",
    ...
    def _display_generation_success(
        self,
        profile: ArtistProfile | None,
        avatar: AvatarBlueprint | None,
        manifest_path: Path,
    ...
    def _update_suggestions(self, profile: ArtistProfile, draft: PersonaWizardDraft) -> None:
        ...
        try:
            ...
            self.console.print(
                f" Warning: unable to persist initial track draft ({exc}).",
                style=self.theme.warning_style,
            )
    ...
    def handle_select(self) -> None:
        ...
        self.console.print(
            f"\n{self.theme.weyland_yutani_tag} ACCESSING ARCHIVES", style=self.theme.warning_style
        )
        ...
        while True:
            ...
            choice_prompt = Text()
            choice_prompt.append(f"{self.theme.radar_blip} ", style=self.theme.warning_style)
            choice_prompt.append(
                "Select persona number or Q to cancel", style=self.theme.info_style
            )
            choice_prompt.append(": ", style=self.theme.warning_style)
            ...
            if not choice.isdigit():
                self.console.print(
                    "  Enter a valid number or Q to cancel.", style=self.theme.warning_style
                )
                continue
            ...
            if index < 0 or index >= len(records):
                self.console.print(
                    "  Invalid selection. Try again.", style=self.theme.warning_style
                )
                continue
            ...
            if self.confirm_with_theme("Load this persona?"):
                if self._apply_persona_selection(record):
                    success_text = Text()
                    success_text.append(
                        f"{self.theme.weyland_yutani_tag} ", style=self.theme.warning_style
                    )
                    success_text.append(
                    ...
                    return
                self.console.print(
                    "  Unable to load persona; select another entry.",
                    style=self.theme.warning_style,
                )
    ...
    def _display_persona_list(self, records: list[PersonaRecord]) -> None:
        ...
        for idx, record in enumerate(records, 1):
            ...
            table.add_row(str(idx), str(name), str(slug), str(lyric), self.theme.radar_blip)
    ...
    def _load_persona_records(self) -> list[PersonaRecord]:
        ...
        for path in sorted(artists_dir.glob("*.json")):
            try:
                ...
                self.console.print(
                    f"  Skipping corrupt manifest {path.name}: {exc}",
                    style=self.theme.warning_style,
                )
        return records
    ...
    def _apply_persona_selection(self, record: PersonaRecord) -> bool:
        ...
        try:
            ...
            self.console.print(
                f"  Manifest validation failed: {exc}", style=self.theme.warning_style
            )
            return False
    ...
    def handle_workbench(self) -> None:
        ...
        if profile is None:
            no_persona_panel = Panel(
                Text(
                    "No persona selected. Generate or select a profile first.",
                    style=self.theme.info_style,
                ),
        ...
        self.console.print(
            f"\n{self.theme.weyland_yutani_tag} TRACK FORGE ONLINE", style=self.theme.warning_style
        )
        ...
        while True:
            ...
            if user_input.startswith("/"):
                ...
                if lowered == "adopt":
                    if not pending_fields:
                        self.console.print(
                            " No pending suggestion to adopt.", style=self.theme.accent_style
                        )
                        continue
                    ...
                    if not applied:
                        self.console.print(
                            " Nothing applied from the suggestion.", style=self.theme.accent_style
                        )
                        pending_fields = None
                        continue
                    ...
                    self.console.print(
                        f" Adopted: {', '.join(applied)}.", style=self.theme.info_style
                    )
                    continue
                ...
                if lowered.startswith("edit"):
                    ...
                    if not remainder:
                        self.console.print(
                            " Usage: /edit field: value", style=self.theme.accent_style
                        )
                        continue
                    if ":" not in remainder:
                        self.console.print(
                            " Use format field: value", style=self.theme.accent_style
                        )
                        continue
                    ...
                    if not field_key:
                        self.console.print(
                            f" Unknown field '{field_name.strip()}'.",
                            style=self.theme.warning_style,
                        )
                        continue
                    ...
                    self.console.print(
                        f" Updated {', '.join(applied)}.", style=self.theme.info_style
                    )
                    continue
                ...
                if lowered == "finalize":
                    ...
                    if not ready:
                        self.console.print(
                            f" Draft not ready: {', '.join(missing)}",
                            style=self.theme.warning_style,
                        )
                        continue
                    ...
                    if self.confirm_with_theme("Render with Suno now?"):
                        self._persist_chat_session(
                            profile, draft, transcript, session_turns, final=True
                        )
                        self._run_creation_engine(profile, draft)
                        return
                    else:
                        self.console.print(
                            " Preview complete. Continue editing or /back to exit.",
                            style=self.theme.info_style,
                        )
                        continue
            ...
            if self.ideation_llm is None:
                self.console.print(
                    " AI assistant not available. Use commands only.", style=self.theme.accent_style
                )
                continue

            # Process AI chat with actual LLM
            self.console.print(
                f"{self.theme.radar_blip} Processing with AI...", style=self.theme.info_style
            )
            ...
            try:
                ...
                self.console.print(
                    f"Assistant: {assistant_response}", style=self.theme.warning_style
                )
    ...
    def _ensure_track_draft(self, profile: ArtistProfile) -> TrackIdeationDraft:
        ...
        if (
            self.session_state.track_draft
            and self.session_state.track_draft.persona_slug == profile.slug
        ):
        ...
        try:
            ...
            self.console.print(
                f"Warning: unable to persist track draft ({exc}).", style=self.theme.warning_style
            )
    ...
    def _display_draft_snapshot(
        self, draft: TrackIdeationDraft, pending_fields: dict[str, object] | None
    ) -> None:
        ...
        table.add_row(
            "Lyrics",
            (draft.lyrics[:50] + "...")
            if draft.lyrics and len(draft.lyrics) > 50
            else (draft.lyrics or "—"),
        )
    ...
    def _apply_draft_updates(
        self, draft: TrackIdeationDraft, updates: dict[str, object], mode: str, summary: str | None
    ...
    def _display_suno_preview(self, preview: SunoPayloadPreview) -> None:
        ...
        table.add_row(
            "Lyrics",
            (preview.lyrics[:100] + "...")
            if preview.lyrics and len(preview.lyrics) > 100
            else (preview.lyrics or "—"),
        )
    ...
    def _persist_chat_session(
        ...
        try:
            ...
            self.console.print(
                f"Warning: unable to persist session ({exc}).", style=self.theme.warning_style
            )
        ...
        with Progress(
            ...
            try:
                ...
                if isinstance(result, TrackArtifact):
                    success_table = Table(
                        title="TRACK GENERATION COMPLETE", title_style=self.theme.title_style
                    )
                    ...
                    self.console.print(
                        "Track generation completed successfully.", style=self.theme.info_style
                    )
    ...
    def _create_campaign(self, profile: ArtistProfile) -> None:
        ...
        if platform.lower() not in ["twitter", "instagram", "discord"]:
            self.console.print(
                "Invalid platform. Defaulting to twitter.", style=self.theme.warning_style
            )
            platform = "twitter"
        ...
        if not beats:
            self.console.print(
                "No beats provided. Campaign creation cancelled.", style=self.theme.warning_style
            )
            return
        ...
        try:
            ...
            cadence_minutes = 120
            self.console.print(
                "Invalid cadence. Using default 120 minutes.", style=self.theme.warning_style
            )
        ...
        campaign = SocialCampaign(
            title=title, platform=platform, beats=beats, cadence_minutes=cadence_minutes
        )
    ...
    def _review_campaigns(self, profile: ArtistProfile) -> None:
        ...
        self.console.print(
            f"{self.theme.weyland_yutani_tag} CAMPAIGN ARCHIVES", style=self.theme.warning_style
        )
    ...
    def _schedule_posts(self, profile: ArtistProfile) -> None:
        ...
        self.console.print(
            f"{self.theme.weyland_yutani_tag} POST SCHEDULER", style=self.theme.warning_style
        )
        ...
        schedule_table.add_row(
            "2024-01-15 14:00", "Twitter", "New track dropping tomorrow! 🎵", "PENDING"
        )
        schedule_table.add_row(
            "2024-01-16 09:00", "Instagram", "Behind the scenes studio shot", "PENDING"
        )
        schedule_table.add_row(
            "2024-01-16 18:00", "Twitter", "Track is live! Link in bio", "PENDING"
        )
    ...
    def _show_analytics(self, profile: ArtistProfile) -> None:
        ...
        self.console.print(
            f"{self.theme.weyland_yutani_tag} ANALYTICS DASHBOARD", style=self.theme.warning_style
        )
    ...
    def handle_quit(self) -> None:
        ...
        if self.confirm_with_theme("Initiate sleep cycle?"):
            ...
            self.console.print()
            self.console.print(
                f"{self.theme.alien_head} Pleasant dreams...", style=self.theme.accent_style
            )
